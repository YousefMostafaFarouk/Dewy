...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    const bool user_clicked = hovered && ioptr.MouseClicked[0];
    ...
    if ((init_state && g.ActiveId != id) || init_changed_specs)
    {
        ...
        if (!is_multiline)
        {
            ...
            if (input_requested_by_nav && (!recycle_state || !(g.NavActivateFlags & ImGuiActivateFlags_TryToPreserveState)))
                select_all = true;
            if (input_requested_by_tabbing || (user_clicked && ioptr.KeyCtrl))
                select_all = true;
        }
    ...
    const bool is_osx = ioptr.ConfigMacOSXBehaviors;
    ...
    if (g.ActiveId == id && ioptr.MouseClicked[0] && !init_state && !init_make_active) //-V560
    ...
    if (g.ActiveId == id)
    {
        ...
        g.ActiveIdAllowOverlap = !ioptr.MouseDown[0];

        // Edit in progress
        const float mouse_x = (ioptr.MousePos.x - frame_bb.Min.x - style.FramePadding.x) + state->ScrollX;
        const float mouse_y = (is_multiline ? (ioptr.MousePos.y - draw_window->DC.CursorPos.y) : (g.FontSize * 0.5f));
        ...
        else if (hovered && ioptr.MouseClickedCount[0] >= 2 && !ioptr.KeyShift)
        {
            stb_textedit_click(state, &state->Stb, mouse_x, mouse_y);
            const int multiclick_count = (ioptr.MouseClickedCount[0] - 2);
        ...
        else if (ioptr.MouseClicked[0] && !state->SelectedAllMouseLock)
        {
            if (hovered)
            {
                if (ioptr.KeyShift)
        ...
        else if (ioptr.MouseDown[0] && !state->SelectedAllMouseLock && (ioptr.MouseDelta.x != 0.0f || ioptr.MouseDelta.y != 0.0f))
        {
            stb_textedit_drag(state, &state->Stb, mouse_x, mouse_y);
            state->CursorAnimReset();
            state->CursorFollow = true;
        }
        if (state->SelectedAllMouseLock && !ioptr.MouseDown[0])
        ...
        const bool ignore_char_inputs = (ioptr.KeyCtrl && !ioptr.KeyAlt) || (is_osx && ioptr.KeySuper);
        if (ioptr.InputQueueCharacters.Size > 0)
        {
            if (!ignore_char_inputs && !is_readonly && !input_requested_by_nav)
                for (int n = 0; n < ioptr.InputQueueCharacters.Size; n++)
                {
                    // Insert character if they pass filtering
                    unsigned int c = (unsigned int)ioptr.InputQueueCharacters[n];
            ...
            // Consume characters
            ioptr.InputQueueCharacters.resize(0);
        }
    }
    ...
    if (g.ActiveId == id && !g.ActiveIdIsJustActivated && !clear_active_id)
    {
        ...
        const int k_mask = (ioptr.KeyShift ? STB_TEXTEDIT_K_SHIFT : 0);
        const bool is_wordmove_key_down = is_osx ? ioptr.KeyAlt : ioptr.KeyCtrl;                     // OS X style: Text editing cursor movement using Alt instead of Ctrl
        const bool is_startend_key_down = is_osx && ioptr.KeySuper && !ioptr.KeyCtrl && !ioptr.KeyAlt;  // OS X style: Line/Text Start and End using Cmd+Arrows instead of Home/End
        ...
        const bool nav_gamepad_active = (ioptr.ConfigFlags & ImGuiConfigFlags_NavEnableGamepad) != 0 && (ioptr.BackendFlags & ImGuiBackendFlags_HasGamepad) != 0;
        ...
        else if (IsKeyPressed(ImGuiKey_UpArrow) && is_multiline)     { if (ioptr.KeyCtrl) SetScrollY(draw_window, ImMax(draw_window->Scroll.y - g.FontSize, 0.0f)); else state->OnKeyPressed((is_startend_key_down ? STB_TEXTEDIT_K_TEXTSTART : STB_TEXTEDIT_K_UP) | k_mask); }
        else if (IsKeyPressed(ImGuiKey_DownArrow) && is_multiline)   { if (ioptr.KeyCtrl) SetScrollY(draw_window, ImMin(draw_window->Scroll.y + g.FontSize, GetScrollMaxY())); else state->OnKeyPressed((is_startend_key_down ? STB_TEXTEDIT_K_TEXTEND : STB_TEXTEDIT_K_DOWN) | k_mask); }
        else if (IsKeyPressed(ImGuiKey_PageUp) && is_multiline)      { state->OnKeyPressed(STB_TEXTEDIT_K_PGUP | k_mask); scroll_y -= row_count_per_page * g.FontSize; }
        else if (IsKeyPressed(ImGuiKey_PageDown) && is_multiline)    { state->OnKeyPressed(STB_TEXTEDIT_K_PGDOWN | k_mask); scroll_y += row_count_per_page * g.FontSize; }
        else if (IsKeyPressed(ImGuiKey_Home))                        { state->OnKeyPressed(ioptr.KeyCtrl ? STB_TEXTEDIT_K_TEXTSTART | k_mask : STB_TEXTEDIT_K_LINESTART | k_mask); }
        else if (IsKeyPressed(ImGuiKey_End))                         { state->OnKeyPressed(ioptr.KeyCtrl ? STB_TEXTEDIT_K_TEXTEND | k_mask : STB_TEXTEDIT_K_LINEEND | k_mask); }
        ...
        else if (IsKeyPressed(ImGuiKey_Backspace) && !is_readonly)
        {
            if (!state->HasSelection())
            {
                ...
                else if (is_osx && ioptr.KeySuper && !ioptr.KeyAlt && !ioptr.KeyCtrl)
        ...
        else if (is_enter_pressed || is_gamepad_validate)
        {
            ...
            if (!is_multiline || is_gamepad_validate || (ctrl_enter_for_new_line && !ioptr.KeyCtrl) || (!ctrl_enter_for_new_line && ioptr.KeyCtrl))
            {
                validated = true;
                if (ioptr.ConfigInputTextEnterKeepActive && !is_multiline)
    ...
    if (render_cursor || render_selection)
    {
        ...
        if (render_cursor)
        {
            state->CursorAnim += ioptr.DeltaTime;
...
bool ImGui::ColorPicker4(const char* label, float col[4], ImGuiColorEditFlags flags, const float* ref_col)
{
    ...
    else if (flags & ImGuiColorEditFlags_PickerHueBar)
    {
        ...
        if (IsItemActive())
        {
            S = ImSaturate((ioptr.MousePos.x - picker_pos.x) / (sv_picker_size - 1));
            V = 1.0f - ImSaturate((ioptr.MousePos.y - picker_pos.y) / (sv_picker_size - 1));
        ...
        if (IsItemActive())
        {
            H = ImSaturate((ioptr.MousePos.y - picker_pos.y) / (sv_picker_size - 1));
            value_changed = value_changed_h = true;
        }
    }
    ...
    if (alpha_bar)
    {
        ...
        if (IsItemActive())
        {
            col[3] = 1.0f - ImSaturate((ioptr.MousePos.y - picker_pos.y) / (sv_picker_size - 1));
            value_changed = true;
        }
    }
