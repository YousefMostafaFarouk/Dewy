...
bool    ImGui_ImplOpenGL3_Init(const char* glsl_version)
{
    ...
    IM_ASSERT(ioptr.BackendRendererUserData == nullptr && "Already initialized a renderer backend!");
    ...
    ioptr.BackendRendererUserData = (void*)bd;
    ioptr.BackendRendererName = "imgui_impl_opengl3";
    ...
    */
    ...
    if (bd->GlVersion >= 320)
        ioptr.BackendFlags |= ImGuiBackendFlags_RendererHasVtxOffset;  // We can honor the ImDrawCmd::VtxOffset field, allowing for large meshes.
#endif
    ioptr.BackendFlags |= ImGuiBackendFlags_RendererHasViewports;  // We can create multi-viewports on the Renderer side (optional)
...
void    ImGui_ImplOpenGL3_Shutdown()
{
    ...
    ioptr.BackendRendererName = nullptr;
    ioptr.BackendRendererUserData = nullptr;
    ioptr.BackendFlags &= ~(ImGuiBackendFlags_RendererHasVtxOffset | ImGuiBackendFlags_RendererHasViewports);
    IM_DELETE(bd);
}
...
bool ImGui_ImplOpenGL3_CreateFontsTexture()
{
    ...
    int width, height;
    ioptr.Fonts->GetTexDataAsRGBA32(&pixels, &width, &height);   // Load as RGBA 32-bit (75% of the memory is wasted, but default font is so small) because it is more likely to be compatible with user's existing shaders. If your ImTextureId represent a higher-level concept than just a GL texture id, consider calling GetTexDataAsAlpha8() instead to save on GPU memory.
    ...
    ioptr.Fonts->SetTexID((ImTextureID)(intptr_t)bd->FontTexture);
