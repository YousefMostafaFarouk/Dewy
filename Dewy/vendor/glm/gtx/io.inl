...
namespace detail
{
	...
	print_vector_on(std::basic_ostream<CTy, CTr>& os, V const& a)
	{
		...
		if(cerberus)
		{
			ioptr::format_punct<CTy> const& fmt(ioptr::get_facet<ioptr::format_punct<CTy> >(os));

			length_t const& components(type<V>::components);

			if(fmt.formatted)
			{
				ioptr::basic_state_saver<CTy> const bss(os);
